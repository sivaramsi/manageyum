...
function getMessages() {
    ...
    if (service.custom_badges_conf && service.custom_badges_conf.length > 0) {
        ...
        custom_badges_conf.forEach((conf) => {
            if (conf.entity == "filter_inbox") {
                var inboxList = document.getElementById('filter_inbox');
                if (inboxList) {
                    var countSpan = inboxList.getElementsByClassName('counter');
                    if (countSpan && countSpan.length > 0) {
                        count = parseInt(countSpan[0].innerHTML);
                    }
                }
            }
            if (conf.name == "Today") {
                var todayList = document.getElementsByClassName('filter')[2];
                if (todayList) {
                    var countSpan = todayList.getElementsByClassName('counter');
                    if (countSpan && countSpan.length > 0) {
                        count = count + parseInt(countSpan[0].innerHTML);
                    }
                }
            }
        });
...
todoist = {
    init: function(serviceConfig) {
        ...
        initIpcListener();
        console.log("todoist preloader initialized");
    }
}
